package laboratory_problems.problem_05;//package midterms.m1;
...
class University {
    ...
    public University mergeFourSmallestDepartments() {
        ...
        if (smallestFour.size() < 4) return this;

        Optional<Department> merged = smallestFour.stream()
        ...
        List<Department> updated = Stream.concat(
                        departments.stream().filter(d -> !smallestFour.contains(d)),
                        merged.stream()
                )
                .collect(Collectors.toList());
    ...
    public List<Department> getDepartmentsWithAtLeastOneHardCourse(int difficultyThreshold) {
        return departments.stream()                .filter(d -> d.getCourses().stream()
