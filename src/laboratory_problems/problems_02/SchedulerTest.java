...
class Scheduler<T> {
    ...
    public Scheduler() {
        timestamps = new ArrayList<Timestamp<T>>(0);
    }
    ...
    public Timestamp<T> last() {
        ...
        Timestamp<T> result = null;
        for (Timestamp<T> t : timestamps) {
            if (t.getTime().isBefore(now) || t.getTime().equals(now)) {
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end) {
        List<Timestamp<T>> returnable = new ArrayList<>(0);
        for (Timestamp<T> t : timestamps) {
    ...
    static public <T, R> Scheduler<R> map(Scheduler<T> source, Function<? super T, ? extends R> mapper, Predicate<? super T> filter) {
        return null;
    }
}
